...
| Job | Tool | Runs on |
|---|---|---|
| `lint` | ruff (lint + format check, auto-fix committed back) | all events |
| `typecheck` | mypy | all events |
| `test` | pytest | all events |
| `release` | python-semantic-release | push to `main` only |
| `docker` | multi-stage build, smoke test, Trivy scan, SBOM, Cosign, GHCR push | all events |
...
Every image pushed to GHCR is:

- **Signed** with [Cosign](https://docs.sigstore.dev/cosign/overview/) using keyless signing via GitHub OIDC — no private key required
- **Attested** with an SPDX SBOM generated by Trivy, stored in the registry alongside the image
- Built with **provenance** and **SBOM** attestations via `docker/build-push-action`
